# auxbookmark - PPx aux: path Web Bookmark bridge
...
8. **URL重複チェック機能**:
   - 登録されている Web ブックマークの URL を正規化して重複を検出し、重複アイテムを「ゴミ箱」または「重複」フォルダへ退避できます。
9. **閲覧履歴の統合とブックマーク化**:
...
- **フォルダ複写 (`C`)**: 反対画面のフォルダ（別ブラウザも可）へ複製します。
  - ※同一フォルダかつタイトルとURLの両方が一致する完全同一アイテムへの複写は、重複防止のため中止されます（別フォルダへの複写や、別名での複製は可能です）。
...
- **名前変更・URL変更 (`R`)**: ブックマークやフォルダの名前、またはリンク先URLを変更します。
  - 通常の文字列を入力した場合は「タイトルの変更」になります。
  - `http://` または `https://` で始まる文字列を入力した場合は「リンク先URLの更新」になります。
  - 変更前と完全に同一（変化なし）の場合は処理が中止されます。
- **削除 (`D`)**:
  - 通常フォルダ内: 「ゴミ箱」へ移動（退避）
  - ゴミ箱フォルダ内: 完全消去
...
### 3. 重複チェック (`dedup`)
登録されている Web ブックマークの URL を正規化（スキーム・ホストの小文字化、末尾スラッシュの統一）して重複を検出し、レポート表示や退避を行います。

- **検出範囲**:
  - プロファイル指定時（例: `auxbookmark dedup Brave`）は、「ブックマーク バー」「その他のブックマーク」「ブックマーク メニュー」配下の**全フォルダを横断して重複を検出**します（別々のフォルダに同じURLが存在する場合も検出対象となります）。
  - 特定のフォルダ内のみを対象にしたい場合は、パス（例: `auxbookmark dedup "Brave/ブックマーク バー"`）を指定します。
- **保持・退避の基準**:
  - 走査順（「ブックマーク バー」→「その他のブックマーク」→「ブックマーク メニュー」）で**先に見つかった1件目を保持（Keep）**し、別フォルダを含む2件目以降を重複（退避対象）として扱います。

```powershell
# レポート表示のみ（データは変更しません）
auxbookmark dedup Brave
auxbookmark dedup "Firefox/ブックマーク バー"

# 重複したブックマーク（2件目以降）を自動的に「ゴミ箱」フォルダへ退避
auxbookmark dedup Brave --trash

# 重複したブックマーク（2件目以降）を「重複」フォルダへ隔離退避
auxbookmark dedup Brave --isolate
```
